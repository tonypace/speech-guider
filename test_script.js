    ...
    console.log('=== VOCAL TRACT SCRIPT LOADING ===');
...
window.startHighlightAnimation = startHighlightAnimation;
    ...
    console.log('=== VOCAL TRACT CLASS DEFINED ===');

    // Helper function to find elements
    window.getGradioElement = function(id) {
        let el = document.getElementById(id);
        if (el) return el;

        const gradioApp = document.querySelector('gradio-app');
        if (gradioApp && gradioApp.shadowRoot) {
            el = gradioApp.shadowRoot.getElementById(id);
            if (el) return el;
        }
        ...
        for (let elem of allElements) {
            if (elem.shadowRoot) {
                el = elem.shadowRoot.getElementById(id);
                if (el) return el;
            }
        }
    ...
    window.updateVocalTractDescriptions = function(incorrectHtml, correctHtml, animationJson, highlightJson) {
        const leftDesc = window.getGradioElement('left-description');
        const rightDesc = window.getGradioElement('right-description');
        const leftPhoneme = window.getGradioElement('left-phoneme');
        const rightPhoneme = window.getGradioElement('right-phoneme');

        const safeIncorrect = incorrectHtml || "No error selected";
        const safeCorrect = correctHtml || "No error selected";

        if (leftDesc) { leftDesc.innerHTML = safeIncorrect; }
        if (rightDesc) { rightDesc.innerHTML = safeCorrect; }

        const leftMatch = safeIncorrect.match(/\/([^/]+)\//);
        if (leftMatch && leftPhoneme) { leftPhoneme.textContent = leftMatch[1]; }
        const rightMatch = safeCorrect.match(/\/([^/]+)\//);
        if (rightMatch && rightPhoneme) { rightPhoneme.textContent = rightMatch[1]; }

        try {
            if (animationJson && animationJson.trim().length > 0) {
        ...
        try {
            if (highlightJson && highlightJson.trim().length > 0) {
    ...
    window.initializeVocalTracts = function() {
        const leftCanvas = window.getGradioElement('vocal-tract-left');
        const rightCanvas = window.getGradioElement('vocal-tract-right');

        if (leftCanvas && rightCanvas && typeof VocalTract !== 'undefined') {
            try {
                ...
                console.log('Vocal tracts initialized successfully');
            } catch (e) {
                console.error('Error initializing vocal tracts:', e);
            }
        } else {
            window.initAttempts++;
            if (window.initAttempts < 40) {
                setTimeout(window.initializeVocalTracts, 500);
            }
        }
    };

    document.addEventListener('DOMContentLoaded', function() {
        setTimeout(window.initializeVocalTracts, 1000);
    });
